...
- キャラクター／作品タグは、デフォルトでよく共起する一般タグ（外見）を表示します。Jaccard の関連タグに戻す場合は設定の **Appearance Tags for Characters** をオフにしてください
...
- **Appearance Tags for Characters**: 有効にすると、キャラクター／作品タグでは Jaccard 関連タグではなく、よく共起する一般タグ（髪色・目・服装など）を表示します
