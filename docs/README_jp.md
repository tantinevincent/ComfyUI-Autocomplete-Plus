...
- 表示位置はデフォルトで現在行の下（オートコンプリートと同様）です。下に十分な空きが無い場合は上に出ます
...
- キャラクター／作品タグは、デフォルトで出現頻度の高い一般・キャラクター・作品タグを表示します。Jaccard の関連タグに戻す場合は設定の **Appearance Tags for Characters** をオフにしてください
...
- **Default Display Position**: ComfyUI起動時のデフォルト表示位置（`vertical` = 現在行の上下（オートコンプリートと同様）、`horizontal` = テキストエリアの左右）
